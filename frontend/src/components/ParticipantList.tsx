    ...
    <div className="flex flex-col h-full bg-transparent">
      ...
      <div className="p-3.5 border-b border-white/[0.08]">
        <div className="flex items-center justify-between mb-2.5">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-xs text-white">Participants</h3>
            <span className="text-[11px] px-1.5 py-0.2 rounded bg-white/[0.05] text-cyan-400 font-mono">
              {(participants || []).length}
            </span>
          </div>
          {isCurrentUserHost && (
            <span className="text-[10px] text-amber-400 font-medium bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded flex items-center gap-1">
        ...
        <div className="relative">
          <input
            ...
            className="w-full glass-input rounded-lg px-3 py-1.5 pl-8 text-xs text-white placeholder-neutral-500 focus:outline-none"
          />
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
        {filteredParticipants.length === 0 ? (
          <div className="text-center py-8 text-neutral-500 text-xs">No participants found</div>
        ) : (
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {isSelf && (
                        <span className="text-[9px] font-semibold text-cyan-400 bg-cyan-500/10 px-1 py-0.2 rounded">
                          YOU
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      {!isHost && !isMod && (
                        <span className="flex items-center gap-1 text-[10px] text-neutral-400">
                          <User className="w-3 h-3 text-neutral-500" /> Viewer
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg border text-xs transition-all cursor-pointer ${
                        isMod
                          ...
                          : 'bg-white/[0.04] text-neutral-400 border-white/5 hover:text-cyan-300'
                      }`}
                    >
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-white/[0.04] hover:bg-white/10 text-neutral-300 border border-white/5 transition-all cursor-pointer"
                      >
                      ...
                      {activeMenuId === p.id && (
                        <div className="absolute right-0 top-full mt-1.5 z-30 w-44 bg-[#181a26] border border-white/10 rounded-xl shadow-2xl p-1.5 space-y-1">
                          <button
                            onClick={() => handleToggleModerator(p)}
                            className="w-full flex items-center gap-2 px-2.5 py-1.5 text-xs text-neutral-200 hover:bg-white/10 rounded-lg transition-colors text-left cursor-pointer"
                          >
                          ...
                          <button
                            ...
                            className="w-full flex items-center gap-2 px-2.5 py-1.5 text-xs text-amber-300 hover:bg-amber-500/10 rounded-lg transition-colors text-left cursor-pointer"
                          >
                          ...
                          <button
                            ...
                            className="w-full flex items-center gap-2 px-2.5 py-1.5 text-xs text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors text-left font-medium cursor-pointer"
                          >
      ...
      {confirmHostTransferId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#161826] border border-amber-500/40 rounded-2xl max-w-sm w-full p-5 shadow-2xl">
            ...
            <div className="flex justify-end gap-2">
              <button
                ...
                className="px-3 py-1.5 rounded-xl text-xs font-semibold text-neutral-400 hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={() => handleTransferHost(confirmHostTransferId)}
                className="px-3 py-1.5 rounded-xl text-xs font-bold bg-amber-500 hover:bg-amber-400 text-black shadow-md shadow-amber-500/20 transition-all cursor-pointer"
              >
                Confirm Transfer
              </button>
            </div>
