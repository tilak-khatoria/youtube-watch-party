...
export const VideoSelectorModal: React.FC<VideoSelectorModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/80 backdrop-blur-md">
      <div className="bg-white dark:bg-[#121420] border border-slate-200 dark:border-white/10 rounded-2xl max-w-lg w-full p-6 shadow-2xl relative overflow-hidden">
        {/* Decorative Glow */}
        ...
        {/* Modal Header */}
        <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-white/10 relative z-10">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-lg text-slate-900 dark:text-white">Change Video</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Load a YouTube video for everyone</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4 relative z-10">
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-slate-100 dark:bg-slate-900/90 border border-slate-300/80 dark:border-white/10 focus:border-rose-500 rounded-xl px-4 py-3 pl-10 text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-rose-500/30 transition-all font-mono"
              />
          ...
          <button
            type="submit"
            className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-rose-600 to-indigo-600 hover:from-rose-500 hover:to-indigo-500 text-white font-bold text-sm shadow-lg shadow-rose-600/25 transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
          >
            Load Video for Room
          </button>
        </form>
              ...
              <button
                ...
                className={`flex items-start gap-2.5 p-2.5 rounded-xl border text-left transition-all cursor-pointer ${
                  currentVideoId === video.id
                    ...
                    : 'border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-900/60 hover:bg-slate-100 dark:hover:bg-slate-800/80 hover:border-slate-300 dark:hover:border-white/15'
                }`}
              >
                <Film className="w-4 h-4 text-slate-400 shrink-0 mt-0.5" />
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 truncate">{video.title}</p>
