...
export const VideoSelectorModal: React.FC<VideoSelectorModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="bg-black border border-white/[0.08] rounded-2xl max-w-lg w-full p-6 shadow-oled-2xl relative overflow-hidden">
        {/* Modal Header */}
        <div className="flex items-center justify-between pb-3.5 border-b border-white/[0.08]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-semibold text-sm text-white">Change Video</h3>
              <p className="text-xs text-neutral-400">Load a YouTube video for everyone</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-3.5">
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full glass-input rounded-lg px-3.5 py-2 pl-9 text-xs font-mono text-white placeholder-neutral-500 focus:outline-none"
              />
          ...
          <button
            type="submit"
            className="w-full py-2.5 px-4 rounded-lg bg-cyan-500 hover:bg-cyan-400 active:scale-[0.98] text-black font-semibold text-xs transition-all cursor-pointer shadow-sm hover:scale-[1.01]"
          >
            Load Video for Room
          </button>
        </form>
        ...
        <div className="mt-5 pt-4 border-t border-white/[0.08]">
          <div className="flex items-center gap-1.5 text-xs font-medium text-neutral-400 mb-2.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {PRESET_VIDEOS.map((video) => (
              ...
              >
                <Film className="w-3.5 h-3.5 text-neutral-400 shrink-0 mt-0.5" />
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-medium text-neutral-200 truncate">{video.title}</p>
                  <p className="text-[10px] text-neutral-500">{video.channel}</p>
                </div>
              </button>
            ))}
          </div>
